...
async function sheetToData({
  auth,
  client,
  spreadsheetId,
  google = googleApisInstance,
}) {
  // create sheets client if not provided
  if (!client) {
  ...
  const titles = data.sheets
    .map(d => {
      ...
      // determine whether there is a sheet type, defaulting to TABLE_TYPE if not
  ...
  const { data: batchData } = await client.spreadsheets.values.batchGet({
    spreadsheetId,
    ranges,
    majorDimension: 'ROWS',
